...
Deno.test("solutions page — shows a player's solve and links to replay", async () => {
  ...
  try {
    ...
    await addSolution({ puzzleSlug: "lasse", moves });

    const solutionsPage = await new SolutionsPage(page).goto("lasse");
    ...
    await expect(page).toHaveURL(/\/puzzles\/lasse\/solutions\/.+/);
  } finally {
    await teardown();
  }
});
